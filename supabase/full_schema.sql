-- ============================================================
-- IELTS Training Platform — v2 完整 Schema（含建表+函数）
-- 用户名+密码登录 + 密钥绑定 + 会话管理
...
CREATE TABLE students (
  ...
  bound_key_id    UUID,                         -- 绑定的密钥 ID（外键在密钥表创建后添加）
...
-- ── 2.5 添加 students 外键（access_keys 表创建后）──
ALTER TABLE students ADD CONSTRAINT fk_students_bound_key
  FOREIGN KEY (bound_key_id) REFERENCES access_keys(id);
...
RETURNS TABLE(out_id TEXT, out_username TEXT) AS $$
BEGIN
  IF EXISTS (SELECT 1 FROM students WHERE students.username = p_username) THEN
...
BEGIN
  ...
  WHERE mt.total_items > 0   -- 只返回有内容的模块
     OR COALESCE(sms.total_answers, 0) > 0  -- 或该学生在此模块有练习数据
